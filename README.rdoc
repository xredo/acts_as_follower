...
To see is a model that acts_as_followable is followed by a model that acts_as_follower use the following
  ...
  # Returns false if the current instance is blocked by the passed record or no follow is 

To give rights to a follower call the following
  book.give_rights(user)
  # Marks the follower as a follower with rights in the followable object. The followers with rights could be retrieved by book.followers_with_rights. You cannot give rights to a blocked user.
...
Copyright (c) 2008 - 2012 ( tom dot cocca at gmail dot com ), released under the MIT license.
